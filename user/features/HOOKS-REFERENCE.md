...
| PostToolUse | Да | `<system-reminder>` | Работает (Issue #15345 подтверждает) |
...
| Feedback после tool | `additionalContext` | PostToolUse (работает стабильно) |
...
| `additionalContext` (PostToolUse) | Средняя | Работает (Issue #15345 подтверждает) |
