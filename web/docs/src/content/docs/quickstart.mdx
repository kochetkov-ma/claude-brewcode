...
claude plugin install brewcode@claude-brewcode</code></pre>
  ...
  <li class="step step-primary">
    <div>
      <strong>Set up the task board</strong> <Badge variant="primary" text="/brewtools:task-board-init" />
      <p>Run it once per repo and accept the spec layer when asked:</p>
      <pre><code>/brewtools:task-board-init</code></pre>
      ...
      <p><strong>What happens:</strong> <code>task-board-init</code> analyzes the repo with parallel agents and
...
<CardGrid>
  <Card title="brewcode" icon="building" href="/brewcode/overview/">
    8 skills: semantic search, deep review, conventions, rules, teams, and more.
  </Card>
  ...
  <Card title="brewdoc" icon="document" href="/brewdoc/overview/">
    6 skills for documentation: doc-staleness tracking, doc generation,
