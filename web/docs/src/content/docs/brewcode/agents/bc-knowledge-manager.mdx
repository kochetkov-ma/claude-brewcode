...
description: "Compacts KNOWLEDGE.jsonl (Ингредиенты): deduplicates entries, merges similar, enforces priority order. Spawned before handoff or when the coordinator reports duplicates."
order: 1211
---
import { Badge, Callout, Card, CardGrid, Steps, UpdateNotice } from '../../../../components/mdx';

# Keep your Ingredients clean and compact.
...
- **Before a session handoff** — compact Ингредиенты (KNOWLEDGE.jsonl) so the next session starts with a clean, prioritized file
