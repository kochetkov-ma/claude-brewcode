...
<Callout type="tip">
**hook-creator knows all 33 events, 5 hook types, and the full routing matrix.** Describe what you want in plain language — it picks the right event, output channel, and schema, then delivers a working file with fail-safe error handling included.
</Callout>

<Callout type="note" title="What's new in 6.2.0">
Two new events: `PreModelSwitch` / `PostModelSwitch` (CC 2.1.251) — the only pair that gates something besides a tool call or a stop.
</Callout>
...
| Reference version | CC 2.1.269 — 33 events, 5 hook types |
...
| PreModelSwitch | Yes | Allow / deny / ask before a model switch (auto-routing, `/model`, cost-based downgrade); timeout blocks the switch — the only blocking event whose timeout blocks instead of failing open (CC 2.1.251) |
| PostModelSwitch | No | Reacts after the switch already happened; same fields plus `source` also `auto`/`resume` (CC 2.1.251) |
...
Hook Creator keeps the event catalog, routing matrix, type/config precedence, environment variables, templates, and changelog out of this page's body — same on-demand pattern the source file itself uses. Read the one that matches what you're doing; skip the rest.

| File | What it holds |
|------|---------------|
| [`hooks-events.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/hooks-events.md) | Full 33-event table, session lifecycle order, matcher syntax, sync/async behavior |
| [`hooks-io-contract.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/hooks-io-contract.md) | Routing matrix, exit-code tables, every output schema, the 10,000-char output cap |
| [`hooks-types-config.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/hooks-types-config.md) | Type/field catalog for `command`/`http`/`mcp_tool`/`prompt`/`agent`, config precedence |
| [`hooks-env.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/hooks-env.md) | Every environment variable a hook process sees, resolving the project root inside a hook |
| [`hooks-templates.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/hooks-templates.md) | Bash/JS skeletons, fail-safe design, the pre-ship validation checklist |
| [`hooks-changes.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/hooks-changes.md) | What changed 2.1.234 -> 2.1.269, full version history, known bugs |

## Return Contract

Verdict first, `<=30 lines`, `path:line`. No hook bodies, no stdin/stdout payload dumps, no <code>CLAUDE_DEBUG</code> transcripts, no preamble — one block per hook (file, event, matcher, purpose, routing, config location, test-fire result). The verification checklist above is the gate, not something to transcribe into the return; debug logs, full payloads, and failing runs go to <code>.claude/reports/&lt;YYYYMMDD-HHMMSS&gt;_hook-creator/</code>, path only.
