...
Wording-only refresh — no new capability, no section added or removed:
- Return Contract now leads the agent definition, ahead of Scope & Checkpoints — matches the current prompting order (role → return contract → scope → procedure → tables)
- Role sentence reworded for density: "Writes bash/sh scripts for macOS/Linux..." replaces the older "Creates production-quality..." phrasing
- All 10 numbered sections (Conventions through Deliverable), `tools`, and `maxTurns: 60` are unchanged
- No Delegation or Scope Fit section was added here — bash-expert works solo end to end, writing and validating its own scripts
</Callout>
...
Verdict first, `<=30 lines`, `path:line`. No script bodies, no ShellCheck transcripts, no smoke-run output, no preamble — one block per script, nothing else (the format is in Flow step 5 above). Failures return the check that failed plus the offending <code>path:line</code>, never the full output; long logs and full ShellCheck runs go to <code>.claude/reports/&lt;YYYYMMDD-HHMMSS&gt;_bash-expert/</code>, path only.
