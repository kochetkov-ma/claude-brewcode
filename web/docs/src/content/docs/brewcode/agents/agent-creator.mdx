...
The agent body was rewritten against Claude Code 2.1.269:
- Body cut from 580 to 128 lines: inline sections were replaced with a `## Read on demand` table pointing at 5 new reference files
- New frontmatter field `experimental.cacheTtl` — see below
- `maxTurns` abort now returns a partial result (2.1.246) — see below
- New `## Delegation` section names `brewtools:text-optimizer` as the only delegation target
- `background: true` now hard-errors on a teammate-spawned agent since CC 2.1.269, instead of silently forcing foreground
</Callout>
...
Agent Creator delegates exactly two kinds of work: the parallel Explore fan-out for repo analysis, and the final optimize pass to `brewtools:text-optimizer`. Everything else that fits in a handful of tool calls happens inline — it never spawns a subagent just to verify its own output, and fans out once rather than nesting spawns.
...
<Steps>
  ...
  <li>
    <div>
      ...
      <p>Checks name format (<code>[a-z0-9-]+</code>), description keyword density, tool minimality (least privilege), no duplicated CLAUDE.md rules in body, unique name in scope, the <code>Return Contract</code> block present, and <code>Scope Fit</code> present exactly when the agent writes code.</p>
    </div>
  </li>
  <li>
    <div>
...
experimental: {cacheTtl: "5m"} # OPT: LOCAL-ONLY -- per-agent prompt-cache TTL, "5m"|"1h" (CC 2.1.248+)
...
| Local only — ignored for plugin agents | `permissionMode`, `hooks`, `mcpServers`, `experimental.cacheTtl` (four keys total; `cacheTtl` since CC 2.1.248) |
...
| File | What it holds |
|------|---------------|
| [`agent-frontmatter-fields.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/agent-frontmatter-fields.md) | All 17 frontmatter fields (2 required, 15 optional) — including `experimental.cacheTtl` |
| [`agent-scope-and-tools.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/agent-scope-and-tools.md) | Deciding `tools:`, where a generated file should live, model precedence, delegation patterns |
| [`agent-context-and-execution.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/agent-context-and-execution.md) | What a subagent inherits from its parent, execution modes, turn/token/concurrency limits |
| [`agent-template.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/agent-template.md) | Description budget, system prompt structure, the Guardrails block, the Validation Checklist |
| [`agent-known-issues.md`](https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents/references/agent-known-issues.md) | Known bugs, the 2.1.234-2.1.269 changelog |

## Return Contract

Verdict first, `<=30 lines`, `path:line`. No agent bodies, no pasted frontmatter, no analysis transcripts, no preamble. Per generated agent: file path, one-line role, <code>model</code>/<code>maxTurns</code>/<code>tools</code> on one line, validation verdict (pass, or the failing checklist item), text-optimizer run or skipped, and any assumption made about the brief. Longer material — analysis notes, generated bodies, full validation runs — goes to <code>.claude/reports/&lt;YYYYMMDD-HHMMSS&gt;_agent-creator/</code>; only the path comes back.

This is the same discipline Agent Creator bakes into every generated agent — see the Return Contract row in the Guardrails table above, a separate contract from this one. [`/brewtools:agent-return-setup`](/brewtools/skills/agent-return-setup/) enforces this at ~1000 / ~2500 est-tokens when installed; the contract itself ships unconditionally.

<CardGrid>
  <Card title="Skill Creator" icon="puzzle" href="/brewcode/agents/skill-creator/">
    Same design process for skills instead of agents.
  </Card>
  <Card title="Agent Deadline" icon="clock" href="/brewtools/skills/agent-deadline-setup/">
