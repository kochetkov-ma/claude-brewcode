...
order: 1205
---
import { Badge, Callout, Card, CardGrid, Steps, Tabs, TabItem } from '../../../../components/mdx';
...
<CardGrid>
  <Card title="SKILL.md" icon="document">
    Required. Frontmatter metadata + instructions for the agent.
  </Card>
  <Card title="references/" icon="book">
    Optional. Detailed docs loaded on demand -- mode-specific instructions, API specs.
  </Card>
  <Card title="scripts/" icon="wrench">
    Optional. Executable utilities -- validation, setup, data processing.
  </Card>
  <Card title="agents/" icon="people">
    Optional. Subagent prompt files (convention, not auto-discovered by Claude Code).
  </Card>
</CardGrid>
...
<CardGrid>
  <Card title="Progressive Disclosure" icon="list">
    3 loading levels: L1 name+desc (always), L2 SKILL.md body (on trigger), L3 references (on demand)
  </Card>
  <Card title="Reference Splitting" icon="document">
    Multi-mode skills split into references/mode.md files. Detect mode, load only matching reference.
  </Card>
  <Card title="Agents-as-References" icon="people">
    Subagent prompts as .md files in agents/ dir. Coordinator passes path, subagent reads itself. 0 tokens in coordinator.
  </Card>
  <Card title="Context Fork" icon="forward-slash">
    context: fork for isolated execution. Fresh context, no history. Best for standalone tasks under 4 phases.
  </Card>
</CardGrid>
...
<Tabs>
  ...
  **Exception -- passing path to agent via Task:** use `$BC_PLUGIN_ROOT` (agent has no `${CLAUDE_SKILL_DIR}`):
  ...
  <Steps>
  <li>
  **Understand** -- Extract workflow from conversation history or ask 2-3 clarifying questions. Determine invocation type.

  If the current conversation already contains a workflow the user wants to capture (e.g., "turn this into a skill"), extract from history first: tools used, steps taken, corrections made, input/output formats, edge cases.

  Confirm extracted workflow with user before proceeding.

  **Invocation Type (CRITICAL)** -- If unclear who will invoke the skill, ASK using AskUserQuestion:
  ...
  <li>
  **Plan Contents** -- Identify what the skill needs:
  ...
  This step is orchestrated by the <a href="/brewcode/skills/skills/">Skills skill</a> (Phase 2).
  </li>
  <li>
  **Create Structure** -- `mkdir -p .claude/skills/skill-name/{references,scripts,assets}`
  </li>
  <li>
  **Configure** -- Set context mode, model, tools, invocation control:
  ...
  <li>
  **Validate** -- Structure + activation checklists. Unit tests for scripts. README generation.
  ...
  Generate unit tests for scripts in `scripts/` directory. Skip if no scripts exist. Create `tests/` directory with `tests/test-{script-name}.sh` for each script. Run all tests, fix failures (max 2 cycles).

  ### README Generation

  Generate `README.md` using the template from `$BC_PLUGIN_ROOT/skills/skills/references/readme-template.md`. Remove inapplicable sections. Keep under 100 lines.
  </li>
  <li>
  **Iterate** -- Refine based on real-world usage feedback. Check Claude's thinking to verify triggering.

  After running test cases, read transcripts. If all runs independently wrote similar helper scripts or took the same multi-step approach -- bundle it: write the common script once in `scripts/`, reference from SKILL.md.
  </li>
  </Steps>
...
<Callout type="tip" title="See Also">
  <ul>
    <li><a href="/brewcode/skills/skills/">Skills skill</a> -- orchestrates this agent with discovery, user interaction, review, and E2E testing</li>
    <li><a href="/brewcode/skills/">All Skills</a> -- overview of all brewcode skills</li>
  </ul>
</Callout>
...
<CardGrid>
  ...
  <Card title="View on GitHub" icon="link" href="https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/agents/skill-creator.md">
