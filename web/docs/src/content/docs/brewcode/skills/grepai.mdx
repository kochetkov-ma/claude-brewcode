...
<Callout type="caution" title="Prerequisites">
  Needs Homebrew, Ollama, and the grepai CLI. Run `/brewcode:grepai setup` — it checks for each component and stops with a clear error if anything is missing.
</Callout>
...
<CardGrid>
  <Card title="spec skill" icon="document" href="/brewcode/skills/spec/">
    Write a SPEC.md — research agents lean on grepai to find reusable code fast.
  </Card>
