...
superreview skill required — what `/brewcode:teams` calls). Both honor the same env overrides
...
| `emit-agent` | — | Writes only `.claude/agents/intent-guard.md`; no superreview skill involved. Used by `/brewcode:teams` |
...
.claude/agents/
  intent-guard.md                   # anti-drift agent, create-or-reuse, shared with /brewcode:teams
```

<CardGrid>
  <Card title="task-board-init skill" icon="document" href="/brewtools/skills/task-board-init/">
