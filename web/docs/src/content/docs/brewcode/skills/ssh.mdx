...
/brewcode:ssh connect to vps-main

# Execute tasks (default mode)
/brewcode:ssh check disk space and docker status

# Deploy operations
/brewcode:ssh pull latest image and restart brewcode-docs

# Update server inventory
/brewcode:ssh update agent
```
...
<Tabs>
  <TabItem label="System Admin">
    ...
    /brewcode:ssh check disk usage on vps-main

    # View memory and CPU load
    /brewcode:ssh show memory and cpu on staging

    # Tail application logs
    /brewcode:ssh show last 100 lines of nginx error log

    # Check for pending OS updates
    /brewcode:ssh list available security updates on vps-main
    ```
  </TabItem>
  <TabItem label="Docker">
    ```bash
    # List running containers
    /brewcode:ssh show docker containers on vps-main

    # View container logs
    /brewcode:ssh show logs for brewcode-docs container

    # Compose operations (requires confirmation)
    /brewcode:ssh docker compose up -d on staging

    # Registry authentication
    /brewcode:ssh login to ghcr.io on vps-main
    ```
  </TabItem>
  <TabItem label="Deploy">
    ```bash
    # Pull latest image and restart
    /brewcode:ssh pull ghcr.io/org/app:latest and restart on vps-main

    # Blue-green deploy
    /brewcode:ssh deploy new version of docs with zero downtime

    # Verify deployment health
    /brewcode:ssh check health endpoint after deploy on vps-main
    ```
  </TabItem>
  <TabItem label="Security">
    ```bash
    # Check firewall rules
    /brewcode:ssh show ufw status on vps-main

    # Review fail2ban status
    /brewcode:ssh check fail2ban banned IPs

    # SSH hardening audit
    /brewcode:ssh check sshd config for security issues

    # Review active SSH sessions
    /brewcode:ssh show who is connected via SSH
    ```
  </TabItem>
</Tabs>
...
| Server not found in inventory | Run `/brewcode:ssh setup` to add the server or `/brewcode:ssh update-agent` to refresh |
