...
Model mapping: `sonnet` to `sonnet`, `opus` to `opus`, `haiku` to `haiku`, `inherit` to omitting `model:` field entirely.
...
### Step 2: Spawn [agent-creator](/brewcode/agents/agent-creator/)
...
> **STOP if NOT\_FOUND** -- report error and list available agents.
...
<CardGrid>
  ...
  <Card title="View on GitHub" icon="link" href="https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/agents">
