...
| review | "ревью" / "review" / "validate" / "проверь корректность" | Single `brewcode:reviewer`, two-phase: review, then double-check the findings |
...
<CardGrid>
  ...
  <Card title="memory" icon="sync" href="/brewdoc/skills/memory/">Sibling: same non-growth sync over memory files, CLAUDE.md, rules</Card>
