---
title: "Spec"
description: "Creates SPEC.md task specification through parallel codebase research and user interaction. Hand the spec to the developer agent to implement."
order: 1102
---
...
**Skipping spec means rewriting later.** Without a specification, the agents implementing a feature have no shared ground truth — they duplicate work, contradict each other, and miss edge cases. The fix costs more than the spec would have.
</Callout>
...
Next Step:
  Hand SPEC.md to the developer agent to implement.
```
...
<Steps>
  <li>
    <div>
      ...
      <p>Verifies <code>.claude/tasks/templates/SPEC.md.template</code> exists. If no project-adapted template is present, the skill falls back to the plugin base template rather than writing a spec into the wrong structure.</p>
    </div>
  </li>
  <li>
    <div>
...
<CardGrid>
  <Card title="developer agent" icon="rocket" href="/brewcode/agents/developer/">
    Next step — hand SPEC.md to the developer agent to implement the feature.
  </Card>
  <Card title="grepai" icon="document" href="/brewcode/skills/grepai/">
    Semantic search — set it up so research agents find reusable code fast.
  </Card>
  ...
  <Card title="Brewcode overview" icon="rocket" href="/brewcode/">
    Full brewcode plugin — spec, grepai, and the agent creator.
  </Card>
</CardGrid>
