...
| Arguments | `[task-path]` defaults to ref in `.claude/TASK.md` (first line = active path) |
| Model | opus |
| Context | session |
...
- If neither -- STOP: `No task path! Run: /brewcode:spec "description" then /brewcode:plan` (see [spec](/brewcode/skills/spec/), [plan](/brewcode/skills/plan/))
  ...
  c. Per task:
     ...
     -- If agent FAILED (is_error=true):
        ...
        b. Persist failure to KNOWLEDGE.jsonl:
           {"ts":"...","t":"x","txt":"Phase {N} agent failed: {error_summary}. Attempt {I}/{MAX}.","src":"manager"}
...
<Callout type="note" title="Escalation after repeated failures">
...
2. Persist to KNOWLEDGE.jsonl:
   `{"ts":"...","t":"x","txt":"Phase N permanently failed: reason","src":"manager"}`
...
<CardGrid>
  ...
  <Card title="View on GitHub" icon="link" href="https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewcode/skills/start">
