...
<Badge variant="primary" text="sonnet" /> <Badge variant="secondary" text="fork" />

## Quick reference

| Field | Value |
|-------|-------|
| Command | `/brewcode:install` |
| Arguments | (no args) -- interactive installer for brew, coreutils, jq, grepai |
| Model | sonnet |
| Context | fork |
...
<Callout type="warning" title="Required">
  If cancelled -- STOP: "Installation cancelled. timeout command required."
</Callout>
