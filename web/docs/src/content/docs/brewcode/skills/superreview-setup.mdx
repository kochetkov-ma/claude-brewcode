...
shape [`/brewtools:agent-return-setup`](/brewtools/skills/agent-return-setup/) enforces at the
token level when that guard is installed.

<Callout type="warning" title="Known limitation: an existing intent-guard.md does not pick this up on its own">
