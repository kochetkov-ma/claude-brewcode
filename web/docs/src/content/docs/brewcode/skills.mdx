...
The same brief lives in the runtime templates these skills read — the `e2e` mode references, `convention`'s rules guide, `spec`'s SPEC-creation reference, and `superreview`'s emitted skill template — so a fan-out three levels deep still gets a full brief.
