...
**Starting from scratch is overwhelming.** The brewcode suite has 4 plugins, 25 skills, and 13 agents. Reading all the docs at once doesn't work — there's no thread, no order, no feedback. Without a guide, most users discover 20% of the features and miss the rest.
</Callout>
...
Docs Site     UP          200
Latest        v4.2.4      GitHub
brewcode      4.2.4       current
brewdoc       4.2.4       current
brewtools     4.2.4       current
brewui        4.2.4       current
Auto-update   ON
...
<Steps>
  ...
  <li>
    <div>
      ...
      <p>Invokes <code>/brewtools:plugin-update check</code>. A plugin counts as stale when it is missing or older than the marketplace version; if all four are current the guide moves on without a word. Otherwise it offers update now, remind later, or skip.</p>
    </div>
  </li>
  <li>
    <div>
      <strong>Load language and progress</strong>
      <p>First run asks your preferred language (English, Русский, Português). Returning users see their completion score and the recommended next topic based on what they've already covered.</p>
    </div>
  </li>
  <li>
    <div>
...
<CardGrid>
  ...
  <Card title="docsync" icon="sync" href="/brewdoc/skills/docsync/">
    Track doc staleness as your project evolves after the guide.
  </Card>
