...
Adds `references/prompting-guide.md` (18 merged rules) and a prompt-quality step — see [Prompt quality pass](#prompt-quality-pass).
</Callout>
...
<Steps>
  ...
  <li><div><strong>Phase 0 — read emit material</strong><p>Reads its own five emit-material files — <code>references/SKILL.md.template</code>, <code>memory-guide.md</code>, <code>agent-audit.md</code>, <code>hard-sync.md</code>, <code>prompting-guide.md</code>. Missing material is a hard stop — never improvised.</p></div></li>
...
Every sweep also runs a rule table against instruction files only — `CLAUDE.md` at any depth, `.claude/rules/*.md`, the `AGENTS.md` family, and agent/skill bodies. Code and docs are never touched; that stays owned by `/docs`. The table lives in `references/prompting-guide.md` — 18 rules merged from current Claude 5 prompting practice and OpenAI/Codex's `AGENTS.md` guidance, each with a grep-able or eyeballed `detect` signal and a bad → good `rewrite`.
