...
Config is saved to `.claude/md-to-pdf.config.json`, always carrying `version` / `generated_by` / `last_updated` (resolved from the plugin's own `plugin.json`, never hardcoded) alongside the two style keys. Example:
