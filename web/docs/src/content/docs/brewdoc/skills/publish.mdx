...
| Requires | `jq` on `PATH`; `zip` as well for directory uploads |
...
<Steps>
  ...
  <li>
    <div>
      ...
      <p>Every block first gates on <code>command -v jq</code> (plus <code>command -v zip</code> on the directory path) and aborts with <code>FAILED: jq required</code> rather than half-publishing. For sites, zips the directory to a temp file, POSTs the archive, then removes the temp file. The owner token returned by the API is never printed — only appended to <code>.claude/brewpage-history.md</code>.</p>
    </div>
  </li>
  <li>
    <div>
