...
The rule applies to every `Explore` / `general-purpose` / `reviewer` spawn in all three modes: a big task handed to one agent = an agent gone for an hour. You cannot observe it, cannot correct it, and it usually drifts off-target.
...
<CardGrid>
  ...
  <Card title="memory" icon="brain" href="/brewdoc/skills/memory/">
    Sync memory, CLAUDE.md (incl. nested), rules and conventions — delete-first, non-growth.
  </Card>
  ...
  <Card title="brewdoc overview" icon="document" href="/brewdoc/overview/">
    All brewdoc skills — docsync, my-claude, memory, md-to-pdf, guide, publish.
  </Card>
</CardGrid>
