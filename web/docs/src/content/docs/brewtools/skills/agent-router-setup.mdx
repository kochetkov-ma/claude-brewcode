...
<Badge variant="primary" text="sonnet" /> <Badge variant="accent" text="status / install / upgrade / enable / disable / uninstall / purge" /> <Badge variant="secondary" text="project only" /> <Badge variant="accent" text="opt-in" /> <Badge variant="warning" text="experimental" />
...
<Steps>
  ...
  <li><div><strong>Refreshed status printed</strong><p>The status block re-runs. Wiring changes (install/upgrade/level/uninstall/purge) need a new Claude Code session; config value changes (<code>enabled</code>, <code>genericTypes</code>, <code>minScore</code>, <code>margin</code>, <code>intents</code>) are read live.</p></div></li>
</Steps>
...
| `version` / `generated_by` / `last_updated` | stamped by `install`/`upgrade`; `/brewcode:setup-status` reads them to flag a stale install |
...
`install`/`upgrade` write `agent-router.json`'s `version`, `generated_by: "brewtools:agent-router-setup"` and `last_updated` at top level. `agent-router.mjs` carries the matching `// brewcode-meta: version=X.Y.Z generated_by=brewtools:agent-router-setup` comment on line 2. `upgrade` re-stamps both, which is how `/brewcode:setup-status` tells an install running an older brewtools apart from the currently installed plugin version.
...
<CardGrid>
  ...
  <Card title="GitHub source" icon="link" href="https://github.com/kochetkov-ma/claude-brewcode/tree/main/brewtools/skills/agent-router-setup">
