...
<Callout type="tip">
**NEW — DeepSeek V4 (April 24, 2026)** — added as priority default. `deepseek-v4-pro` (1.6T MoE, 1M context, thinking modes), [top open-source on SWE-bench](https://www.swebench.com/), verified prompt caching (90% discount, automatic). Alias `claudedeepseek`. No compat flags required.
...
<Callout type="tip">
**One command fixes it.** Run `claudedeepseek` or `claudeglm` — Claude Code switches to DeepSeek V4, GLM-5.2, or another backup. Cost: ~$10/month backup budget. New terminal = back on Anthropic.
</Callout>
...
claudeglm       # GLM-5.2 — #1 SWE-bench Pro, 200K ctx
claudeqwen      # Qwen 3.7 Plus — 1M context window
claudeminimax    # MiniMax-M3 — cheapest per token
...
<CardGrid>
  <Card title="DeepSeek V4" icon="rocket" href="https://platform.deepseek.com">
    <Badge variant="accent" text="Priority" /> <Badge variant="accent" text="Top open-source" /> <Badge variant="accent" text="Cache verified" /> **deepseek-v4-pro** — 1.6T MoE, 1M context, thinking modes. SWE-bench Verified 80.6%, Pro 55.4%. 90% cache discount, automatic. Alias `claudedeepseek`. No compat flags. [Leaderboard](https://www.swebench.com/).
  </Card>
  <Card title="Z.ai / GLM" icon="star" href="https://z.ai/subscribe">
    <Badge variant="accent" text="Strong coding perf" /> <Badge variant="accent" text="Cache verified" /> **glm-5.2** — $1.40 / $4.40 per 1M tokens. 200K context. Backend auto-cache (native).
  </Card>
  ...
  <Card title="MiniMax" icon="money" href="https://platform.minimax.io">
    <Badge variant="accent" text="Cache verified" /> **MiniMax-M3** — $0.30 / $1.20 per 1M tokens. 200K context. Cheapest option with documented cache pricing.
  </Card>
