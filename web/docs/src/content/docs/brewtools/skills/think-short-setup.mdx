...
description: "Install, upgrade or remove terse-mode hooks that inject brevity directives at session start, every 20th prompt, and into subagents - project or global scope."
order: 3127
---
...
The counter hook's cadence and the directive body were both tightened — see the directive text below and [How it works](#how-it-works) for the every-20th-prompt schedule.
</Callout>
...
```
Be terse: results first, no preamble/filler/sycophancy, ASCII only. Think
short - minimal internal reasoning, no exploring aloud. Grep before Read,
Edit over Write, parallel calls in one message. Plan the full edit set, then
execute.

Keep code simple, no over-engineering: before writing anything new, find the
closest well-built repo counterpart and extend it - additive to
conventions/rules/docs, never instead. After writing code, one simplify pass.

Comment like a human, not an AI: only where it earns it (non-obvious logic,
public APIs, class/method docstrings - JavaDoc/PyDoc/etc). No narrating
self-evident code or line-by-line noise. Each comment ~1 line, 2 only if
asked.
```
...
| Output-heavy sessions where you want periodic reinforcement, not constant injection | Install project-scope — counter hook fires every 20 turns, not every turn |
...
| `think-short-prompt-counter.mjs` | UserPromptSubmit | Injects the directive on every 20th user prompt (turns 20, 40, 60 — never turn 1, which is covered by SessionStart) |
| `think-short-subagent.mjs` | SubagentStart | Injects the full directive into every spawned subagent via `additionalContext` |

The counter hook keeps injection periodic, not per-turn. Turns 2–19 run without re-injection; at turn 20 the hook fires again to reinforce brevity after context drift.
...
`injects` measures the subagent hook only. SessionStart and the every-20th-prompt injection are separate paths.
