...
**Board drain.** When the `progress` count on the board reaches zero, `task-tracker` appends `NEXT: run /brewtools:task-board-init <path> upgrade` — but only when this repo has no `.claude/skills/task-spec/` yet. A board that already has the spec layer stays silent, because `upgrade` forces `SPEC_MODE=on` and would otherwise push the spec layer onto a repo that deliberately declined it.
