...
**Natural language (resolves to `on` + `profile aggressive`):**
```
go all in
```

> The skill also accepts Russian natural language (e.g. on/off, profile names, "go aggressive"). The complete RU+EN trigger list lives in the skill's SKILL.md.
...
hardcoded defaults: enabled=false, profile=medium
env override: THINK_SHORT_DEFAULT=(unset)
recent log:
  think-short: NL-prompt "go all in" -> resolved as on + profile aggressive
```
...
4. Hardcoded `DEFAULT_THINK_SHORT` constant in `helpers/state.mjs` — `enabled=false`, `profile=medium`, `blacklist=[debate, docs-writer, architect]`
