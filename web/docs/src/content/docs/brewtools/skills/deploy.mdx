...
<CardGrid>
  ...
  <Card title="Deploy to VPS" icon="sync">
    SCP + SSH deployment with health checks and rollback. Integrates with [ssh](/brewtools/skills/ssh/) skill server inventory.
  </Card>
...
<Tabs>
  ...
  <TabItem label="SSH Integration">
    ...
    Run [`/brewtools:ssh`](/brewtools/skills/ssh/) setup first to configure server connections if you need VPS deployments.
  </TabItem>
</Tabs>
...
| VPS deploy failed | Check SSH connectivity with [`/brewtools:ssh`](/brewtools/skills/ssh/) `connect to server-name` |
