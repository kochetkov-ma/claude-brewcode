...
# AI left traces in your code. This removes them.
...
<Callout type="tip">
**One command, any scope.** Pass a commit hash to process every changed file from that session, or pass a path for targeted cleanup. Everything after the first token becomes a custom instruction — no quotes needed. Parallel sub-agents (рецепты, скиллы) handle large folders without blocking.
</Callout>
