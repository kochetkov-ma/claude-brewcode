...
<Steps>
  ...
  <li>
    <div>
      ...
      <p>Commit or folder scope uses the mixed flow dispatcher: files split into blocks by complexity, classified as haiku (simple config/data/text) or sonnet (source with logic, tests, complex SQL). All <code>Task</code> calls fire in one message for true parallelism. Each sub-agent receives its block, the flow rules, and any custom instructions prepended at highest priority. The skill substitutes the <strong>resolved absolute path</strong> of the rules directory into every sub-agent prompt before spawning — subagents get no plugin-root variable of their own, so an unexpanded one would leave them humanizing against no rules at all.</p>
    </div>
  </li>
  <li>
    <div>
