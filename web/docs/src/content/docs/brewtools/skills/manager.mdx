...
<Callout type="tip">
**What is Manager mode?** You type a short codeword (`++m`) anywhere in your message and Claude instantly switches into "delegate-everything orchestrator" mode -- just for that one message. It stops writing code by hand and instead decomposes your task, builds a plan, and delegates every piece to the best specialist agent. No setup required. The codeword is stripped from your message; your actual text is preserved.
</Callout>
...
```
++m сделай рефакторинг модуля оплаты
```
...
```
сделай рефакторинг модуля оплаты
```
...
| Codeword | What it does |
|----------|-------------|
| `++m` | Inject Manager mode for this one message -- Claude orchestrates, never codes by hand |
| `++mp` | Same as `++m` plus Plan Mode -- Claude writes out the full task graph in English before starting |
...
| Run one task under the wall then auto-revert | `refactor payment service в хард режиме` |
| Run task as Manager without changing any wall state | `build release plan от роли менеджера` |
| Check current state | `/brewtools:manager status` or `что сейчас` |
...
On every user message, `manager-prompt.mjs` scans the prompt text. It tests for `++mp` before `++m` to avoid prefix collision. If neither codeword is present (and the wall is not armed), the hook exits immediately -- no file I/O, zero overhead.
...
<details>
<summary>Full command and intent reference (all actions, RU+EN triggers, hard-one-shot vs manager-run)</summary>
...
| `hard-one-shot` | `<task> в хард режиме` / `<task> in hard mode` | Wall ON -- run task as Manager -- auto-revert wall OFF when done (reverts even on failure or abort) |
| `manager-run` | `<task> от роли менеджера` / `<task> as manager` | Run task as Manager with wall UNTOUCHED (discipline via injected prompt only -- no wall toggle) |
...
`hard-one-shot` (`в хард режиме`) actually toggles the guard: wall ON -- task runs -- wall auto-reverts OFF when done, even on failure or abort. The guard never silently persists.

`manager-run` (`от роли менеджера`) does NOT touch the wall state at all. Manager discipline is enforced via the injected prompt only. Use this when you want Manager behavior without arming or changing the guard.
...
<CardGrid>
  ...
  <Card title="Prompt Injection" icon="bolt" href="/brewtools/prompt-injection/">
    How the `++m` / `++mp` codewords inject the Manager contract into the model -- the full mechanism explained.
  </Card>
