---
title: "Manager"
description: "Type ++m / ++rr / ++r anywhere to inject a Manager or review-discipline block for one turn. The HARD wall denies Write/Edit/NotebookEdit/WebFetch/MCP-write in the main session, forcing delegation. RU+EN."
order: 3124
---
...
Detection order: within the review group, `++rr` is tested before `++r` (longest-prefix first). `++m` is matched as a standalone token; in plan mode it auto-adds the plan addon.
...
<CardGrid>
  ...
  <Card title="Prompt Injection" icon="bolt" href="/brewtools/prompt-injection/">
    How the `++m` / `++rr` / `++r` codewords inject the Manager / review contract into the model -- the full mechanism explained.
  </Card>
