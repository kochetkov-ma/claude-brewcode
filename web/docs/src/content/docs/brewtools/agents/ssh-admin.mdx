---
title: "SSH Admin"
description: "Linux server admin agent — SSH, Docker, firewalls, systemd, Caddy/Nginx, security hardening. Triggers: ssh admin, server management, deploy to server."
order: 3212
---
...
- `Return Contract` now opens the agent definition instead of closing it — the model reads the output shape before anything else.
- Every guardrail is unchanged: the six-level safety classification, the `## APPROVAL REQUIRED` envelope fields, and the approval-token rule read the same as before.
- No new capability in this pass — wording and table conversion only.
</Callout>
