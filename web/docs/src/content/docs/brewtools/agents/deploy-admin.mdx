...
- Agent prompt tightened for density — shorter prose, `Return Contract` now opens the agent definition, worked example kept.
- `## GitHub Config`, `## Workflow Inventory`, `## Server Targets`, and `## Secrets` merged into one `## Project Config` table — same facts, same probe/`NEEDS-INPUT` fallbacks, easier to scan.
- The rule that a derived GitHub target still carries into every envelope's `HOST:` field, and is never self-approved for a MODIFY+ operation, is kept in the merged table.
- Safety Rules, Compound Rules, the Release Flow, and the Approval Contract mechanics are all unchanged.
</Callout>
