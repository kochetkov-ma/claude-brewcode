...
The closest-counterpart rule injected by `++m` and `++a` is not unique to this hook -- `/brewcode:teams-setup` and `/brewcode:agents` inject the same rule into generated agents whose domain writes code, scripts, SQL, schemas, or infra. See [Teams](/brewcode/skills/teams-setup/) and [Agents](/brewcode/skills/agents/).
