...
| **Только сессия** | `claude --plugin-dir ./focus-task` |
...
# Абсолютный путь
claude --plugin-dir /path/to/claude-brewcode/focus-task

# Несколько плагинов
claude --plugin-dir ./focus-task --plugin-dir ./plugins/other
```
...
```bash
claude plugin validate ./focus-task
```
