...
- **setup-status: the `installed (version unknown)` state is retired.** Its replacements are specific: `stale (legacy, unstamped)`, `stale (legacy stamp)`, `stale (drift)`, `stale (behind X.Y.Z)`, plus a distinct `version unknown (plugin asset missing)` for the case where the comparison source itself is absent
