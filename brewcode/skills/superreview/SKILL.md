...
project's `.claude/rules` + convention files -> ONE targeted parallel fan-out (domain experts + 2 scope passes) ->
per-finding adversarial VALIDATION gate -> scope gate (AskUserQuestion) -> ONE merged P0-P3 report at
...
| Scope baseline | Task + issue + recorded decisions resolved read-only; no baseline -> `UNKNOWN` and a PERMANENT P2 cap on scope findings |
...
| Scope gate | `AskUserQuestion` on unsanctioned expansion / unproven absence; rewrites priorities only, never adds findings, never lifts the UNKNOWN cap |
