#!/usr/bin/env node
/**
 * brewcode:semble — SessionStart hook (self-contained, installed into a project).
 *
...
const CORRUPT = { systemMessage: 'semble: state file is corrupt — run /brewcode:semble status' };
    ...
    return {
      ...
      hookSpecificOutput: {
        ...
        additionalContext:
          ...
          'Run /brewcode:semble resume before relying on semantic search.',
      },
    };
  }
  if (phase === 'error') {
    return { systemMessage: 'semble: error — run /brewcode:semble status' };
  }
  if (phase === 'ready') {
