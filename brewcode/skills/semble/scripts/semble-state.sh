...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) [ -z "$ARG" ] || { sc_err "unexpected argument: $1"; exit 2; }; ARG="$1" ;;
  esac
  shift
done
...
case "$SUB" in
  ...
  init)
    ...
    PATCH="$(SC_C="$(sc_cache_root_code)" SC_H="$(sc_repo_hash "$(sc_project_root)" || true)" node -e '
process.stdout.write(JSON.stringify({phase:"prereq_ready",enabled:true,scope:"user",
 ...
 resumePrompt:"/brewcode:semble resume"}));')"
    sc_state_patch "$PATCH" >/dev/null
...
process.stdout.write(JSON.stringify(out));'
  ...
  phase)
    ...
    sc_state_patch "$(SC_P="$ARG" node -e 'process.stdout.write(JSON.stringify({phase:process.env.SC_P}))')" >/dev/null
    [ "$JSON" = "1" ] || sc_ok "phase $FROM -> $ARG"
    st_emit "$(SC_F="$FILE" SC_A="$FROM" SC_B="$ARG" node -e 'process.stdout.write(JSON.stringify({schema:1,file:process.env.SC_F,from:process.env.SC_A,to:process.env.SC_B,status:"ok"}))')"
    ;;

  complete)
    [ -n "$ARG" ] || { sc_err "complete needs a STEP"; exit 2; }
    case "$ARG" in
      prereq|mcp|permissions|guidance|agents|warm|smoke) ;;
      *) sc_err "unknown step: $ARG (prereq|mcp|permissions|guidance|agents|warm|smoke)"; exit 2 ;;
    esac
    st_assert_schema
    sc_state_patch "$(SC_S="$ARG" node -e 'process.stdout.write(JSON.stringify({completed:[process.env.SC_S]}))')" >/dev/null
    DONE="$(sc_state_get completed)" || exit 1
    [ "$JSON" = "1" ] || sc_ok "completed += $ARG -> ${DONE:-[]}"
    st_emit "$(SC_F="$FILE" SC_S="$ARG" SC_D="${DONE:-[]}" node -e 'process.stdout.write(JSON.stringify({schema:1,file:process.env.SC_F,step:process.env.SC_S,completed:JSON.parse(process.env.SC_D),status:"ok"}))')"
    ;;
