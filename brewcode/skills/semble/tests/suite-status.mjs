...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  check('13-nextStep', j.nextStep, 'Run /brewcode:semble setup', 'partial must point at setup');
...
const FIXTURE_STATES = [
  ['absent.json', 'absent', 'not_installed', 'Run /brewcode:semble setup'],
  ['correct.json', 'correct', 'partial', 'Run /brewcode:semble setup'],
  ['stale.json', 'stale_args', 'partial', 'Run /brewcode:semble setup'],
  ['wrongscope.json', 'wrong_scope', 'partial', 'Run /brewcode:semble setup'],
  ['duplicate.json', 'duplicate', 'partial', 'Run /brewcode:semble setup'],
  ['upstream.json', 'upstream_unpinned', 'partial', 'Run /brewcode:semble setup'],
];
...
// malformed: exit is still 0, verdict is error, the bad file is named
{
  ...
  check('21-malformed-nextStep', j.nextStep,
    `Fix by hand, then re-run /brewcode:semble status: ${cj}`,
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  const cases = [
    ...
    ['verifying', { phase: 'verifying', enabled: true }, 'verifying', 'Run /brewcode:semble resume'],
    ['disabled', { phase: 'disabled', enabled: false }, 'disabled', 'Run /brewcode:semble enable'],
    ['ready-but-off', { phase: 'ready', enabled: false }, 'disabled', 'Run /brewcode:semble enable'],
    ['error', { phase: 'error', enabled: true }, 'error', 'Run /brewcode:semble setup to repair'],
    ['prereq_ready', { phase: 'prereq_ready', enabled: true }, 'partial', 'Run /brewcode:semble setup'],
  ];
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  check('99-correct-args', correctCfg.args,
    ['--from', 'semble[mcp]==0.5.2', 'semble', '--content', 'code', 'config'],
