#!/usr/bin/env node
// suite-integration.mjs - cross-unit integration for brewcode:semble.
//
...
check('wired: pin', R.pin, { approved: '0.5.2', spec: PIN_SPEC }, 'the approved pin is reported verbatim');
...
check('wired: mcp registered command', R.mcp.scopes.user,
  ...
  }, 'the exact §6.1 server config reached ~/.claude.json through the stub');
...
// state
check('wired: state keys', keysOf(R.state), ['completed', 'enabled', 'phase', 'present', 'updatedAt'],
  'the §9.1 state shape');
...
const p2Pre = p2Settings.hooks.PreToolUse;
p2Settings.hooks.PreToolUse = p2Pre.filter((e) => e.matcher !== 'Grep');
writeFileSync(P2_SETTINGS, `${JSON.stringify(p2Settings, null, 2)}\n`);
check('partial: exactly one Grep entry was removed',
  [p2Pre.length, p2Settings.hooks.PreToolUse.length], [2, 1],
  'the reminder was registered under Bash and Grep; only Grep is gone');
...
check('bare: verdict', [R3.verdict, R3.nextStep], ['not_installed', 'Run /brewcode:semble setup'],
...
check('coverage: buckets (independently known fixture set)',
  ...
  [{ '.py': 1, '.ts': 1 }, { '.properties': 1, '.yaml': 1 },
    { '.html': 1, '.md': 2 }, { '.csv': 1 }],
  'py+ts are code, yaml+properties config, html and both .md docs-only, csv excluded');
check('coverage: totals', R3.coverage.totals,
  {
    code: 2, config: 2, docsOnly: 3, excluded: 1, indexable: 4, classified: 8, unclassified: 0,
...
check('coverage: totals agree with the buckets they summarise',
  [sumOf(R3.coverage.code), sumOf(R3.coverage.config), sumOf(R3.coverage.docsOnly),
    sumOf(R3.coverage.excluded), sumOf(R3.coverage.code) + sumOf(R3.coverage.config),
    sumOf(R3.coverage.code) + sumOf(R3.coverage.config) + sumOf(R3.coverage.docsOnly)
...
check('lifecycle: recorded claude command line',
  ...
  'the exact §6.1 argv reached the claude binary, exactly once');
...
check('lifecycle: disabled verdict', [disReport.verdict, disReport.nextStep],
  ['disabled', 'Run /brewcode:semble enable'], 'status reflects the disabled project immediately');
