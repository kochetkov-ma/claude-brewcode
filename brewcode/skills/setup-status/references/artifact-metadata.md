...
must not report the version that `install` left. Six implementations, all four carriers:
...
| `.claude/agent-return.json` | `brewtools/skills/agent-return-setup/assets/INSTALL.md` - one config block (`:156-216`, stamp `:208`, verify `:214`, hard-fail `:205`) reused by every mode | install `:308`, upgrade `:455-486`, enable+disable `:496` (stamp `:536`, verify `:542`). Contract stated at `:95`; no `level` mode exists here. Written in both scopes - the global twin is `~/.claude/agent-return.json`, Bash-only |
...
construction - `STAMPED_FILES` (33 rows) and `VERSIONED_DOCS` (7 rows) share no path.
...
`STAMPED_FILES` in `.claude/scripts/bump-version.sh:51-83` is the complete set of
mechanism-`a` artifacts - **33 rows** today, `path|kind|generated_by`. A file listed
