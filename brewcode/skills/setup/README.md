...
Checks prerequisites, then analyzes your project's tech stack, testing framework, database layer, and existing agents to generate tailored templates and a review skill under `.claude/tasks/`. Run this once before using any other brewcode skill (`spec`, `review`). Setup handles all required tooling (brew, coreutils, jq) and optionally sets up grepai for semantic code search.
