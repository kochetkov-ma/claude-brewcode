...
Targets: `.claude/agents/*.md`, `*/agents/*.md` (repo-local only). Disabled files (`_name.md`) are skipped and reported. `/brewdoc:memory` applies the same non-growth sync to memory files, CLAUDE.md, rules and conventions; its `full` mode also syncs the agent and skill rosters in-place — use this skill when you want a roster on its own.
