...
case "$TRACE_SCRIPT_DIR" in
  */.claude/teams/*)
    ...
    if [ -f "$TRACE_PROJECT_ROOT/.claude/scripts/toolchain_preflight.py" ] && \
      [ -x "$TRACE_PROJECT_ROOT/.claude/scripts/toolchain_preflight.py" ]; then
      TRACE_NODE_RESOLVER="$TRACE_PROJECT_ROOT/.claude/scripts/toolchain_preflight.py"
    fi
    ;;
esac
