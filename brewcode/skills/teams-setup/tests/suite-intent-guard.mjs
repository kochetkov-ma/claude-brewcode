 ...
 * of .claude/agents/intent-guard.md that /brewcode:teams-setup Phase 3 calls.
...
const IG_REL = '.claude/agents/intent-guard.md';
...
/** A project root with an optional pre-existing intent-guard.md. */
...
const agentNames = (root) => readdirSync(join(root, '.claude', 'agents')).sort();
const backups = (root) =>
  agentNames(root).filter((n) => /^intent-guard\.md\.bak-[0-9]{8}-[0-9]{6}$/.test(n));
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  check('b1.tree', agentNames(root).join(','), 'intent-guard.md', 'no extra file was created');
  check(
    'b1.conflictReported',
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  check(
    ...
    bak.map((n) => read(root, join('.claude', 'agents', n))).join(''),
    body,
...
// ────────────────────────────────────────────────────────────────────────────
{
  ...
  check('b7.tree', agentNames(root).join(','), 'intent-guard.md', 'exactly one file, no backups');
}
