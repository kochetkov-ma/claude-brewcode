...
description: Полное дерево файлов brewcode плагина с описаниями
---
...
```
brewcode/                                    # Корневая директория плагина
│
...
│   ├── bc-coordinator.md                      # Координатор задачи (haiku): статус фаз, извлечение knowledge, NEXT ACTION протокол, inline compaction
│   ├── bc-knowledge-manager.md                # Менеджер знаний (haiku): дедупликация, сортировка, обрезка KNOWLEDGE.jsonl
│   ├── bc-grepai-configurator.md              # Конфигуратор grepai (opus): анализ проекта, генерация config.yaml через 5 параллельных исследований
│   ├── bc-auto-sync-processor.md              # Процессор auto-sync (sonnet): обработка одного документа - анализ, исследование, обновление
│   ├── bc-rules-organizer.md                  # Организатор правил (sonnet): создание/оптимизация .claude/rules/*.md файлов
...
│   │
│   ├── setup/                                 # /brewcode:setup - Инициализация плагина в проекте
...
│   │
│   ├── auto-sync/                             # /brewcode:auto-sync - Универсальная синхронизация документации
...
│   │
│   ├── grepai/                                # /brewcode:grepai - Управление семантическим поиском
...
│   │
│   ├── install/                               # /brewcode:install - Установка зависимостей
...
│   │
│   ├── teardown/                              # /brewcode:teardown - Очистка файлов плагина
│   │   └── SKILL.md                           # Инструкции: удаление templates/, cfg/, skills/brewcode-review/; сохранение задач (haiku, fork)
│   │
│   ├── secrets-scan/                          # /brewcode:secrets-scan - Сканирование на утечку секретов
│   │   └── SKILL.md                           # Инструкции: detect-secrets, TruffleHog, Gitleaks (sonnet, fork)
│   │
│   ├── mcp-config/                            # /brewcode:mcp-config - Управление MCP серверами
│   │   └── SKILL.md                           # Инструкции: status, disable, enable MCP серверов (sonnet, fork)
│   │
│   ├── text-human/                            # /brewcode:text-human - Humanize код и документацию
│   │   └── SKILL.md                           # Инструкции: упрощение AI-генерированного кода (sonnet, fork)
│   │
│   └── text-optimize/                         # /brewcode:text-optimize - Оптимизация текста для LLM
...
│   ├── brewcode.config.json.template        # Шаблон конфигурации: knowledge (validation, retention), logging, agents, constraints, autoSync
│   │
...
│   │   └── post-agent-protocol.md.template    # Напоминание 2-step протокола: WRITE report -> CALL bc-coordinator (условная загрузка по paths)
│   │
│   └── skills/                                # Шаблоны скиллов для целевого проекта
│       └── review/                            # Адаптируемый скилл ревью (копируется при setup)
│           ├── SKILL.md.template              # Шаблон /brewcode:review: кворум, группы, Critic mode, DoubleCheck (opus, fork)
...
└── package.json                               # npm-манифест: claude-plugin-brewcode@2.7.1, скрипты build/publish
```
...
└── .claude/
    ...
    │   │   ├── brewcode.config.json         # Пользовательские настройки: knowledge, logging, agents, constraints, autoSync
    │   │   └── brewcode.state.json          # Состояние между сессиями: текущая задача, последний компакт
    │   │
    │   ├── templates/                         # Адаптированные шаблоны (из /brewcode:setup)
    ...
    │   │
    │   ├── logs/                              # Логи хуков
    │   │   └── brewcode.log                 # Единый лог всех хуков: [info/warn/error] [hook] message
    │   │
    │   ├── reviews/                           # Отчеты код-ревью (/brewcode:review)
    ...
    │
    ├── skills/
    │   └── brewcode-review/                 # Адаптированный скилл ревью (из /brewcode:setup)
    │       ├── SKILL.md                       # Ревью с кворумом, адаптированный под проект
    │       └── references/                    # Промпты и шаблоны отчетов
    │
    └── rules/                                 # Правила Claude Code (из /brewcode:rules)
        ...
        ├── grepai-first.md                    # Приоритет grepai для поиска по коду (из /brewcode:grepai setup)
...
| Агенты | 14 | bc-coordinator, bc-knowledge-manager, bc-grepai-configurator, bc-auto-sync-processor, bc-rules-organizer, agent-creator, skill-creator, bash-expert, hook-creator, text-optimizer, architect, developer, reviewer, tester |
...
|-------|--------|------------|
| bc-coordinator | haiku | Оркестрация: статус, knowledge, NEXT ACTION |
| bc-knowledge-manager | haiku | Компактификация KNOWLEDGE.jsonl |
| bc-grepai-configurator | opus | Анализ проекта, генерация config.yaml |
| bc-auto-sync-processor | sonnet | Обработка документов для auto-sync |
