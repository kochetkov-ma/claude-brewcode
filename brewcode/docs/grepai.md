...
> Semantic code search with automatic AI attention focusing on grepai instead of standard utilities.

**See also:** [README.md](README.md) | [INSTALL.md](INSTALL.md) | [bc-grepai-configurator](agents/bc-grepai-configurator.md) | [/brewcode:grepai](skills/grepai/SKILL.md)

## Overview

grepai is a semantic code search via embeddings. Integrated into brewcode plugin through MCP (Model Context Protocol) with a forced AI attention focusing system.
...
│  ──────────────        ─────────────           ────────────────            │
│  • Ollama (bge-m3)     • /brewcode:grepai    • 3 hooks                   │
│  • grepai CLI          • bc-grepai-configurator • 1 rule template          │
...
│   ├── config.yaml                 # Generated by bc-grepai-configurator
...
│
├── agents/
│   └── bc-grepai-configurator.md   # Opus agent for config generation
│
├── templates/rules/
...
# ATTENTION SYSTEM (CRITICAL)

> A system of 7 injection points ensuring grepai priority over standard Glob/Grep.
...
**Characteristics:**
- Prefix `grepai:` for categorization and parsing
- ALL CAPS `USE...FIRST` for imperative force
- Exact tool name: `grepai_search`
- Context: "for code exploration"
...
│                                                                          │
│  ┌───────────────────────────────────────────────────────────────────┐   │
│  │ INJECTION #6: Root CLAUDE.md (brewcode parent)                  │   │
│  │                                                                   │   │
...
**Trigger conditions:**
- `.grepai/` directory exists
- Ollama is running (`curl localhost:11434/api/tags`)
- `index.gob` exists
- `grepai watch` process is active (or auto-started)

**Output:** systemMessage + additionalContext at session start.
...
**Scope:** ALL agents including system agents:
- Explore, Plan, Bash, general-purpose
- developer, tester, reviewer
- bc-coordinator, custom agents

**Mechanism:** Prepending - reminder becomes the FIRST line of the agent prompt.
...
**Trigger:** Every Glob or Grep tool call.

**Purpose:** Reminder about semantic alternative when using pattern-based search.
...
- First lines have the greatest weight (primacy effect)
- Reminder is seen by agent BEFORE any instructions

---

## Attention Strength Escalation

Version v2.0.57 → v2.0.58 shows intentional strengthening:
...
**Change:** Advisory → Imperative (removed `consider`, added `USE`).
...
# CONTEXT COMPOSITION (What Claude Sees)

> Complete context composition and attention switching mechanism Rule → MCP Tool Descriptions.
