...
auto-sync-type: doc
description: Детальное описание всех команд brewcode плагина
---

# Команды brewcode плагина
...
|---|---------|-----------|----------|--------|-------------|
| 1 | `/brewcode:install` | Установка зависимостей | fork | sonnet | -- |
| 2 | `/brewcode:setup` | Анализ проекта, генерация шаблонов | fork | opus | install |
| 3 | `/brewcode:spec` | Создание спецификации задачи | session | opus | setup |
| 4 | `/brewcode:plan` | Создание плана выполнения | session | opus | spec или Plan Mode |
| 5 | `/brewcode:start` | Запуск выполнения задачи | session | opus | plan |
| 6 | `/brewcode:review` | Код-ревью с кворумом | fork | opus | setup (генерирует скилл) |
| 7 | `/brewcode:rules` | Извлечение правил из знаний | session | sonnet | start (KNOWLEDGE.jsonl) |
| 8 | `/brewcode:auto-sync` | Синхронизация документации | session | opus | setup |
| 9 | `/brewcode:grepai` | Семантический поиск по коду | session | sonnet | install |
| 10 | `/brewcode:teardown` | Удаление файлов плагина | fork | haiku | setup |
| 11 | `/brewcode:secrets-scan` | Поиск секретов и учетных данных | fork | sonnet | -- |
| 12 | `/brewcode:mcp-config` | Управление MCP серверами | session | sonnet | -- |
| 13 | `/brewcode:text-optimize` | Оптимизация текста для LLM | fork | sonnet | -- |
| 14 | `/brewcode:text-human` | Упрощение и гуманизация текста | fork | sonnet | -- |
...
|-------|--------|-----------|
| `bc-coordinator` | haiku | Координация задачи: статусы фаз, валидация, управление отчётами |
| `bc-knowledge-manager` | haiku | Компактификация KNOWLEDGE.jsonl, дедупликация, приоритизация |
| `bc-grepai-configurator` | opus | Генерация `.grepai/config.yaml` через глубокий анализ проекта |
| `bc-auto-sync-processor` | sonnet | Обработка одного документа при auto-sync: анализ, исследование, обновление |
| `bc-rules-organizer` | sonnet | Создание и оптимизация `.claude/rules/*.md` файлов |

---

## 1. `/brewcode:install`

**Назначение:** Интерактивный установщик всех зависимостей, необходимых для работы brewcode плагина. Проверяет и устанавливает brew, coreutils, jq, а также опциональные ollama, bge-m3 и grepai.
...
| **Контекст** | `fork` |
| **Модель** | `opus` |
| **Зависимости** | `/brewcode:install` (рекомендуется) |
...
| `.claude/tasks/cfg/brewcode.config.json` | Конфигурация плагина |
| `.claude/skills/brewcode-review/SKILL.md` | Адаптированный скилл code review |
| `.claude/skills/brewcode-review/references/` | Шаблоны промптов и отчётов для review |
...
| **Контекст** | `session` |
| **Модель** | `opus` |
| **Зависимости** | `/brewcode:setup` (шаблон `SPEC.md.template` должен существовать) |
...
```
/brewcode:spec "Реализовать авторизацию через JWT токены"
...
| **Контекст** | `session` |
| **Модель** | `opus` |
| **Зависимости** | `/brewcode:spec` (или Plan Mode файл) |
| **Allowed tools** | `Read`, `Write`, `Glob`, `Grep`, `Bash`, `Task`, `AskUserQuestion` |
...
| **Контекст** | `session` |
| **Модель** | `opus` |
| **Зависимости** | `/brewcode:plan` (PLAN.md должен существовать) |
...
| Агент | Назначение |
|-------|-----------|
| `bc-coordinator` | Инициализация, обновление статусов, извлечение знаний, валидация |
...
4. **Execute Phases** -- для каждой фазы:
   ...
   - **CALL bc-coordinator** -- чтение отчёта, извлечение знаний
   - Запуск верификационной фазы (тот же 2-step protocol)
5. **Final Review** -- 3+ агентов `reviewer` параллельно
6. **Complete** -- статус `finished`, вызов `/brewcode:rules`
...
2. CALL bc-coordinator --> читает отчёт с диска, извлекает знания
```
...
| **Контекст** | `fork` |
| **Модель** | `opus` |
| **Зависимости** | `/brewcode:setup` (генерирует скилл в `.claude/skills/brewcode-review/`) |
| **Allowed tools** | `Read`, `Glob`, `Grep`, `Task`, `Bash`, `Write` |

**Важно:** Этот скилл не поставляется в плагине напрямую. Он генерируется командой `/brewcode:setup` как проектный скилл в `.claude/skills/brewcode-review/SKILL.md`, адаптированный под конкретный стек проекта.
...
```
/brewcode:review "Проверить null-safety в сервисном слое"
/brewcode:review -q 5-3 -c "Полный review модуля авторизации"
...
| **Контекст** | `session` |
| **Модель** | `sonnet` |
| **Зависимости** | `/brewcode:start` (создаёт KNOWLEDGE.jsonl) или текущая сессия |
...
| **Контекст** | `session` |
| **Модель** | `opus` |
| **Зависимости** | `/brewcode:setup` (конфигурация) |
...
| Агент | Модель | Назначение |
|-------|--------|-----------|
| `bc-auto-sync-processor` | sonnet | Обработка одного документа: анализ, исследование, обновление |
...
| **Контекст** | `session` |
| **Модель** | `sonnet` |
| **Зависимости** | `/brewcode:install` (brew, ollama, grepai установлены) |
...
|-------|--------|-------|-----------|
| `bc-grepai-configurator` | opus | setup, optimize | Анализ проекта, генерация config.yaml |
...
| `optimize` | Бэкап конфига -> регенерация через bc-grepai-configurator -> reindex |
...
**Назначение:** Удаляет все файлы и директории, созданные командой `/brewcode:setup`. Сохраняет директории задач и пользовательские правила.
...
| **Контекст** | `fork` |
| **Модель** | `haiku` |
| **Зависимости** | `/brewcode:setup` (файлы для удаления) |
...
| `.grepai/` | Удаляется |
| `.claude/skills/brewcode-review/` | Удаляется |
