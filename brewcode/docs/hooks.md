...
description: Детальное описание всех хуков brewcode плагина
---
...
## Переменная BC_PLUGIN_ROOT

Путь к корню brewcode плагина.
...
```
BC_PLUGIN_ROOT=/Users/.../.claude/plugins/cache/claude-brewcode/brewcode/2.12.0
```
...
|----------|------------------|
| Скиллы (main conversation) | `$BC_PLUGIN_ROOT` доступен в additionalContext |
| Субагенты | `$BC_PLUGIN_ROOT` инжектируется в prompt |
...
4. Пишет логи в stderr (видны в терминале) и в файл `.claude/tasks/logs/brewcode.log`

### Файл конфигурации

Путь: `.claude/tasks/cfg/brewcode.config.json`
...
| `.claude/tasks/logs/brewcode.log` | append | Лог-файл |
...
- Симлинк LATEST.md используется скиллом `/brewcode:plan` для обнаружения свежего плана
...
| `.claude/tasks/cfg/brewcode.config.json` | read | Конфигурация (maxTokens, системные агенты) |
| `.claude/tasks/logs/brewcode.log` | append | Лог-файл |
...
|---------|-----------|
| Нет `tool_input` | Выход без изменений |
| `subagent_type` == `bc-coordinator` | Привязка сессии к lock-файлу |
...
| Lock нет | Выход без изменений (brewcode не активен) |

**Привязка сессии (координатор):**

При завершении bc-coordinator, если lock существует, но `session_id` не привязан:
...
| `.claude/tasks/cfg/brewcode.config.json` | read | Список системных агентов |
| `.claude/tasks/logs/brewcode.log` | append | Лог-файл |
...
- 2-step протокол обеспечивает вызов bc-coordinator после каждого рабочего агента
...
5. **Обновление состояния:**
   - Записывает в `brewcode.state.json`:
...
| `.claude/tasks/cfg/brewcode.config.json` | read | Конфигурация (maxEntries, maxTokens) |
| `.claude/tasks/cfg/brewcode.state.json` | read + write | Обновление состояния |
| `.claude/tasks/logs/brewcode.log` | append | Лог-файл |
...
| `isCoordinator(agentType)` | post-task | Проверка bc-coordinator |
...
SessionStart -----> session-start.mjs (лог, маппинг, симлинк)
    ...
    v
/brewcode:start создаёт .lock (без session_id)
    |
    v
