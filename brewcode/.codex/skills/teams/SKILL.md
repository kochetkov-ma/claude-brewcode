...
> If `.codex/agents/intent-guard.toml` already exists (e.g. `$brewcode:superreview` created it),
...
4. After each batch, optimize:
   ...
   ```
   > **Never run text-optimize on `.codex/agents/intent-guard.toml`.** Its frontmatter `description`
...
bash "<skill-directory>/../superreview/scripts/generate.sh" emit-agent && echo "OK" || echo "FAILED"
```
