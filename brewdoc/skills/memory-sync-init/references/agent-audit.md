...
**Why it is unconditional.** A roster drifts silently: a tool list that grew and was never trimmed, a description
whose triggers no longer match how people invoke it, an ownership glob pointing at a renamed directory, a handoff
naming an agent that was deleted. None of that appears in a code diff, so nothing but a standing audit finds it.
...
Evidence is gathered read-only:

```bash
ls .claude/agents/*.md .claude/skills/*/SKILL.md            # the live roster
grep -n '^tools:\|^allowed-tools:\|^name:\|^model:' <file>  # declared contract
grep -oE 'mcp__[a-z0-9_]+__' <file> | sort -u               # MCP servers claimed
git ls-files -- '<owned glob>' | head -3                    # ownership glob resolves
```
...
| 2 | `description:` shape | ROLE FIRST in the opening clause, then a concrete trigger list a router can match on | Rewrite role-first; add the real triggers observed in the repo's own invocations |
...
| 4 | `tools:` minimality | Exactly the tools the BODY uses - nothing aspirational | Remove unused tools; a read-only recon agent carrying `Write` / `Edit` is a defect, not a convenience |
...
| 13 | Non-growth | The file did not get longer than it was at the start of the run | Cut to fit; audit findings are fixed by DELETING, not by appending caveats |
...
| 6 | Commands in body | Every command runs in this repo today (the binary exists, the script exists, the manifest exists) | Replace with the real command; unverifiable step -> delete it and REPORT |
...
| 10 | Placeholder hygiene | A generated skill carries no unresolved generation-time placeholder | REPORT it - a leftover slot means the generator run did not finish |
| 11 | Non-growth | The skill file and each reference end at or below their starting line count | Cut to fit |
...
| Unknown key found | REPORT it. Do not strip a key just because you do not recognise it - it may be newer than your knowledge |
...
| Never invent | Do not add a key you cannot point at in the spec or in a sibling file |
...
| Class | Handling |
|-------|----------|
| MECHANICAL - apply the fix | `name:` mismatch, dead path or glob, unused tool entry, missing `Bash` on a searcher, broken reference citation, uncited orphan reference, restated rule replaced by a pointer, date stamp, wording compressed |
| RESPONSIBILITY - report only | Anything that changes WHAT an agent owns or does: widening or narrowing its scope, retargeting its ownership globs to a different subsystem, merging or splitting agents, deleting an agent whose surface is gone, resolving two agents that claim the same seam |
