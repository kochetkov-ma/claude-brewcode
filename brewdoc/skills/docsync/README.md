...
```sh
/brewdoc:docsync                       # first run in a project -> init; otherwise status
/brewdoc:docsync status                # what is tracked / stale
/brewdoc:docsync sync                  # sync stale docs (with confirmation)
/brewdoc:docsync sync --all            # sync every in-scope doc
/brewdoc:docsync reread                # re-read tracked docs to refresh context
/brewdoc:docsync frontmatter           # retro-add frontmatter to in-scope docs (opt-in)
/brewdoc:docsync uninstall             # remove hooks + settings entries (non-destructive)
```
...
|------|---------|--------------|
| init | first run / `init` | Asks threshold (default 7d) + exclude globs; copies 3 hooks into `.claude/hooks/`; writes `.claude/docsync/config.json`; idempotently + non-destructively merges `.claude/settings.json` (backs up to `.bak`). Never adds frontmatter to docs. |
| status | `status` | Lists in-scope docs with `last_updated`, age, and stale/fresh/no-date state. No changes. |
...
| frontmatter | `frontmatter` | Opt-in retro-add of docsync frontmatter to in-scope docs. Never automatic at init. |
...
All `*.md` in the project, minus the `exclude` globs chosen at init and any file
...
Full docs: [docsync](https://doc-claude.brewcode.app/brewdoc/skills/docsync/)
