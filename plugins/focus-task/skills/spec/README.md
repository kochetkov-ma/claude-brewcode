...
```bash
/focus-task:spec "Build authentication system with OAuth2 support"
```

Or provide a requirements file:

```bash
/focus-task:spec /path/to/requirements.md
```
...
```bash
/focus-task:plan .claude/tasks/{TIMESTAMP}_{NAME}_task/
```

## Requirements

- Must run `/focus-task:setup` first (creates templates)
