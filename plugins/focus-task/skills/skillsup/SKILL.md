...
> **Skill Management:** List, improve, create skills with 84% activation rate via forced-eval hook.
...
**Smart Detection:** If first argument is NOT a mode keyword (`list`, `setup`, `up`, `create`), treat entire input as target for `up` mode.

**Examples:**
- `/skillsup` → `list`
- `/skillsup list` → `list`
- `/skillsup setup` → `setup`
...
- `/skillsup plugins/focus-task/skills/setup` → `up`, target=path **(shorthand)**
...
Install forced-eval hook + show skill list.
...
The forced-eval hook uses `UserPromptSubmit` event with `updatedInput` channel:
```
...
This raises activation from 20-50% to **84%** by prepending skill-check reminder to every user prompt.
...
```
Task tool:
  subagent_type: "focus-task:skill-creator"
  prompt: |
