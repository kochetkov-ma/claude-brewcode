...
```bash
/focus-task:skillsup                    # List all skills
/focus-task:skillsup list               # Same as above
/focus-task:skillsup setup              # Install forced-eval hook + list
/focus-task:skillsup up commit          # Improve "commit" skill
/focus-task:skillsup commit             # Shorthand: improve "commit" skill
/focus-task:skillsup plugins/focus-task/skills/setup  # Shorthand: improve by path
/focus-task:skillsup ~/.claude/skills/  # Improve all global skills (folder)
/focus-task:skillsup create "semantic search"  # Create new skill
```
...
- `FT_PLUGIN_ROOT` environment variable (set by focus-task plugin)
- `jq` command-line tool (for hook installation)
- skill-creator agent (bundled with focus-task)
