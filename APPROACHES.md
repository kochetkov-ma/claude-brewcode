...
`hard: true`. `hardmode-guard.mjs:109` answers on `PreToolUse`:
...
subagent passes untouched: subagent stdin carries `agent_id`/`agent_type` and the guard returns `{}` for it.
...
Recorded, not fixed. Each line names the file.

1. `brewtools/hooks/hardmode-guard.mjs` is shipped but absent from `brewtools/hooks/hooks.json`; the CLAUDE.md hook table does not say so, so a reader may assume installing brewtools arms the wall.
2. The hard wall tells a main session from a subagent using the stdin keys `agent_id`/`agent_type`, which the official hook reference does not list (it documents only `subagent_type`/`subagent_id`). `hardmode-guard.mjs:13-34` marks this NOT-IN-DOC: first verified live on CC 2.1.177, last re-checked on 2.1.195, and not re-tested on the current 2.1.226.
3. CLAUDE.md groups agent-router with the "absent key means disabled" family. It is the opposite: `agent-router.mjs:116-117` sets `enabled: true` in `DEFAULTS`, so an absent key and an absent file both mean ENABLED.
4. A router config file that exists but fails to parse turns the feature fully OFF rather than falling back to defaults (`agent-router.mjs:212-216`) - the reverse of agent-return and agent-deadline.
5. CLAUDE.md does not give `hardStopRatio` its default; the code default is `2` (`agent-deadline-guard.mjs:56`) and the key may be omitted.
6. `agent-router-setup/assets/judge-prompt.md` is inlined into `settings.json`, not copied, so a plugin upgrade refreshes the judge text only if the merge block is re-run (`INSTALL.md:288-292`).
7. The codeword text is not in `brewtools/hooks/lib/manager-prompts.mjs` - that file only resolves paths; the five blocks live in `brewtools/skills/manager-setup/references/{full,planmode,architect,review-regression,review-double}.md`.
8. `/brewtools:manager-setup edit` only ever writes mode `full` (`SKILL.md:547`, `const mode = 'full'`), so `++a`, `++rr` and `++r` cannot be customized through the skill.
9. CLAUDE.md calls semble-setup "semble-first rule + hooks". It also writes a CLAUDE.md marker block, `.sembleignore`, a `.gitignore` line, two permission entries and 6 settings entries (`brewcode/skills/semble-setup/SKILL.md:443`).
10. This repo's own `.claude/rules/semble-first.md` is stamped `version: "5.2.0"` while the shipped template is `5.5.2`; body text is identical, so `setup-status` currently reports semble as stale here.
11. CLAUDE.md says the hard wall blocks "main-session Write/Edit/Bash". Also denied by default: `WebFetch`, `Artifact`, `NotebookEdit`, mutating MCP and every unlisted tool; at `strict` also `WebSearch` and all MCP (`hardmode-guard.mjs:78-100`, `:318-339`).
12. `.claude/skills/brewcode-review/SKILL.md` has no `disable-model-invocation` key and its description carries triggers, so the model can auto-activate it - unlike every shipped skill. The stated invariant is scoped to `brew*/skills/`, so this is not a rule violation; flagged as intentional-or-not.
13. Report path divergence: superreview writes `.claude/reports/{TIMESTAMP}_superreview/REPORT.md`, brewcode-review writes `.claude/tasks/reviews/{TIMESTAMP}_{NAME}_report.md`, which does not match the `YYYYMMDD-HHMMSS_<name>/` convention.
14. `intent-guard` rows bypass the adversarial validator by design, arriving pre-verdicted `CONFIRMED-BY-EVIDENCE` (`superreview-setup/references/SKILL.md.template:682-687`).
15. CLAUDE.md lists task-board-setup artefacts as "agent + skill + rule + features". In SPEC_MODE it also emits `.claude/skills/task-spec/SKILL.md` and two spec templates - nine stamped artefacts, not seven (`task-board-setup/SKILL.md:17-25`).
16. CLAUDE.md's docsync line does not mention `state.json` or the `.claude/settings.json` merge, both of which the install performs.
17. Both emitted skills (`task-board`, `memory-sync`) must NOT set `disable-model-invocation`, the opposite of the shipped-skill invariant. Both are intentional (`task-board-setup/references/03-task-board-skill.md:5`, `memory-sync-setup/SKILL.md:347`).
18. `enable`/`disable` for memory-sync is reachable only through `scripts/generate.sh` verbs; the presence of `SKILL.md.disabled` is the entire state, with no config file.
19. Return Contract heading level is not uniform: `brewcode/agents/skill-creator.md:747` uses `#` while the other seven use `##`, so a `grep '^## Return Contract'` audit under-reports 7/8.
20. `brewcode/agents/bc-rules-organizer.md` has no `## Scope guard` and no bridge line before `## Checkpointing`; nothing in `agent-creator.md` marks Scope guard as conditional - only `Scope Fit` is.
21. Frontmatter key order and the canonical setup mode list have no script check. Both hold 27/27 and 11/11 today but a new skill can violate either and still exit 0 from `validate-skill.sh`.
22. `agent-creator.md:405-416` says `magenta` is not a valid `color`, yet `brewtools/agents/text-optimizer.md:6` ships `color: magenta`; three agents (`ssh-admin`, `deploy-admin`, `bc-rules-organizer`) declare no `color` at all.
23. The prompt-contract exemption list is duplicated: prose in `prompt-contract.md` sec. 5 and a hard-coded path in `validate-skill.sh` `is_exempt_skill`. Adding an exemption requires editing both.
24. The word counts in "What is injected at each moment" are read off the source strings by eye. No test asserts the size of any injected block; the only mechanical limit is the 9000-char cap in `forced-eval.mjs:15` and `manager-prompt.mjs:87`.
25. The worked examples quote the payload of one representative hook per moment. Where several hooks answer the same event - four on `SessionStart`, five on `UserPromptSubmit` - the others are described in words, not quoted.
...
short, and always name a concrete file. If a claim here stops matching the code, move it to "Known gaps"
rather than deleting it, so the drift stays visible. If the thing you added is a new injection point - a new
...
CLAUDE.md and the code disagree, the code wins and the disagreement gets a numbered gap line.
