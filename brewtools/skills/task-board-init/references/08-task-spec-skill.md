...
Substitute `{{LANG}}`, `{{REPO_NAME}}`, `{{DOMAINS}}`, `{{FIRST_DOMAIN}}`, `{{EXCLUSIONS}}`, `{{DOMAIN_AGENTS}}`, `{{ARCHITECT_AGENT}}`, `{{TODAY}}`. Write the body below to `TARGET/.claude/skills/task-spec/SKILL.md`.

Emit this file ONLY when `SPEC_MODE=on`. When `off`, skip P3.5 entirely -- nothing here is written.

`{{DOMAIN_AGENTS}}` expands to a COMPLETE markdown table -- header row AND the `|---|` separator -- with columns exactly `agent | domains covered | specialty`, built in generator phase P1 from `TARGET/.claude/agents/**`:
...
If the target repo has no agents, `{{DOMAIN_AGENTS}}` expands to exactly the literal line:
...
argument-hint: "<TASK_ID> [full | design | refresh]"
...
- **Writes ONLY `.claude/features/**`.** Never app code. EXCLUSIONS (never written, never edited): {{EXCLUSIONS}}.
...
| # | Pick |
|---|------|
| 1 | the project domain agent covering that domain (table above) |
| 2 | `{{ARCHITECT_AGENT}}` -- the project architecture-capable agent |
| 3 | built-in `Plan` |

Whichever was used MUST be named per domain in the design's `## Evidence`. A touched domain with no
domain agent MUST be stated explicitly there -- silence is a defect.
...
1. `<TASK_ID>` = first argument; mode = second argument, default `full`. No id given: `Glob` `.claude/features/progress/*.md` then `todo/*.md`; one WIP task -> use it; several -> `AskUserQuestion` with the candidate ids. Never guess.
...
4. Extract `## Scope` rows. No `## Scope` section -> derive `S1..Sn` from the task body, show the proposed table via `AskUserQuestion`, and write it into the task file in P7. Never proceed on an unstated scope.
...
CONTEXT: task Scope ids you must cover: S1 <block>, S3 <block>. Already fixed: <decisions from an
...
CONSUMER: this session merges your proposal into .claude/features/specs/<ID>-design.md, whose
  fixed sections are Context & constraints / Architecture / Data flow / Interfaces & contracts /
  Data model / Failure modes & reliability / Complexity budget / Non-goals / Scope coverage /
  Open architectural questions / Evidence. Answer in those terms so it merges without rewriting.
DONE: components + responsibilities for your domain; the exact interface you expose and the ones
  you require from sibling domains; failure modes and how they are handled; a COMPLEXITY BUDGET --
  what you deliberately do NOT build and why (reliable and project-sized beats clever); which
  Scope ids your slice covers and any it cannot; open architectural questions. Report as those
  headings, terse, {{LANG}}, with file:line evidence. No code, no diffs.
")
\`\`\`
...
Merge the P2 proposals into `specs/<ID>-design.md` following `DESIGN_TEMPLATE.md` section for section (do not restate them here, do not reorder, do not drop).
...
2. `## Complexity budget` is mandatory and never empty: what is deliberately NOT built, and why. Reliable and project-sized beats clever.
3. `## Evidence` names the agent used PER DOMAIN and flags any domain that had no domain agent.
4. FM: `task`, `kind: design`, `status: draft`, `open_questions`, `created`, `updated` ({{TODAY}} on first write; `created` preserved on refresh). Each FM field follows the template's own frontmatter comment.
...
CONTEXT: your own P2 proposal for this domain was merged with <other domains>; the merge may have
  bent it. Task Scope ids: S1..Sn. Open questions currently: Q1 <text> (spec, blocking yes),
  AQ1 <text> (design, blocking yes) ...
...
DONE: a table -- id | severity (blocker|major|minor) | doc + section | claim | evidence file:line
  | suggested fix in one line. Explicitly answer: does the design cover every in-scope id for your
  domain? is anything over-engineered against the Complexity budget? is any interface underspecified
  or any failure mode unhandled? Terse, {{LANG}}, no prose essay.
")
\`\`\`
...
4. Report: docs written, `spec:` value, decisions count, open questions (blocking / non-blocking), G1 verdict, review findings confirmed vs rejected, and the agent used per domain.
...
| G3 | the task's `## Scope` changed after the docs were written | both docs `status: draft`; run `/task-spec <ID> refresh` |
...
| -- | a write targets anything outside `.claude/features/**` | refuse; {{EXCLUSIONS}} are never written |
...
- Procedure (authoritative): `.claude/features/TRACKER.md` section 10
- Templates: `.claude/features/specs/SPEC_TEMPLATE.md`, `.claude/features/specs/DESIGN_TEMPLATE.md`
- Rules: `.claude/rules/tasks.md`
- Board + lifecycle: `/task-board`, agent `task-tracker`
```
