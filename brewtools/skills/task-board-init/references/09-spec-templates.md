...
Emitted only when `{{SPEC_MODE}}` is `on`. Write each block below to its path under `TARGET/.claude/features/specs/`. Substitute `{{REPO_NAME}}`, `{{LANG}}`, `{{TODAY}}` (ISO date), `{{FIRST_DOMAIN}}`.
