...
CONTEXT: nothing has been generated yet; this is the first pass, running in parallel with Agent B
  ...
  AskUserQuestion and may override it before generation.
...
ROLE: you own EXCLUSIONS + LANG + DOCS. Explore and report only -- do NOT create, write or edit any file, and
  do NOT propose domains or a release style (Agent A owns those), and do NOT inventory .claude/agents/**
  (Agent C owns that).
...
CONTEXT: nothing has been generated yet; this is the first pass, running in parallel with Agent A (domains +
  release style) and Agent C (domain-agent inventory). The user reviews your output in an AskUserQuestion
...
SCOPE: in -- read TARGET/.claude/agents/**/*.md (frontmatter name/description/tools + body), plus any
  plugin-provided agents visible to the repo (TARGET/.claude/settings.json enabledPlugins, .claude-plugin/**,
  CLAUDE.md agent tables). Out -- writing anything; judging agent quality; rewriting or creating agents.
CONTEXT: nothing has been generated yet; this is the first pass, running in parallel with Agent A (domains +
  ...
  A's confirmed DOMAINS. The user reviews your output in an AskUserQuestion and may override it.
...
- If TARGET has no .claude/agents/ (or it holds no agent .md), return this exact line instead of a table:
...
Evidence: bullet the paths you read (.claude/agents/*.md, settings.json, CLAUDE.md sections, plugin manifests).
")
```
...
This confirmation also carries the optional CLAUDE.md-optimization question (see SKILL.md P1); ask both in the
same AskUserQuestion, do NOT split into two rounds.
