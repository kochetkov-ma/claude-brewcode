...
| Model | opus |
| Arguments | `[target repo path]` (empty = current dir) `[upgrade]` (optional) `["free-text directive"]` (optional) |
...
A repo that already has `.claude/features/board.md` cannot be re-initialized. Pass the `upgrade` directive to add the spec layer to it:
...
/brewtools:task-board-init ../repo upgrade
```
...
- Refuses a fresh init if `.claude/features/board.md` already exists (board already deployed -- use `/task-board` to operate it, or `upgrade` to add the spec layer).
- `upgrade` without an existing `board.md` is refused too -- there is nothing to upgrade; run a fresh init instead.
