...
function guard(args, cwd, extra = {}) {
  const r = spawnSync('bash', [GUARD, ...args], {
    ...
    env: { ...process.env, CLAUDE_PROJECT_DIR: cwd, ...extra },
  });
...
// ── 1. snapshot: preconditions, permissions, gitignore ──────────────────────
{
  ...
  check('snapshot.rundir-prefix', runDir.startsWith(`${repo}/.claude/reports/`), true,
    'RUN_DIR lives under .claude/reports/');
...
// ── 2. clean-tree guard (the interim zero-code requirement) ─────────────────
{
  ...
  check('dirty.no-rundir', existsSync(join(repo, '.claude', 'reports')), false,
    'nothing is written under .claude/reports/ on refusal');
