...
```
/brewtools:agent-router                  # status (default, no args)
/brewtools:agent-router install          # install — asks level, defaults to fast
/brewtools:agent-router level strict     # add the LLM judge (read the cost first)
/brewtools:agent-router level fast       # drop it again
/brewtools:agent-router disable          # enabled:false, hook stays wired
/brewtools:agent-router enable           # back on
/brewtools:agent-router uninstall        # unwire + delete the hook file, keep config
/brewtools:agent-router purge            # + delete config and tmp markers
/brewtools:agent-router вычисти всё      # free-text intent works (RU+EN) -> purge
```
...
| `level fast` | kept | tier-2 stripped | `level:"fast"` | kept |
| `level strict` | kept | tier-2 appended | `level:"strict"` | kept |
