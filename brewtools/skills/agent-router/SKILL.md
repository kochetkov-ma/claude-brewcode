...
`neverFlag` defaults to FOUR entries — `Explore`, `Plan`, `statusline-setup`, `output-style-setup` — and they are never flagged: `Explore` is the right tool for search, `Plan` for planning.
