...
```
/brewtools:agent-deadline                       # status (default, no args)
/brewtools:agent-deadline install               # install — asks scope + budget
/brewtools:agent-deadline install global 30     # global, 30-minute budget
/brewtools:agent-deadline disable               # enabled:false, files stay
/brewtools:agent-deadline enable                # back on
/brewtools:agent-deadline uninstall             # unwire + delete hook files, keep config
/brewtools:agent-deadline purge                 # + delete config and state
/brewtools:agent-deadline вычисти всё           # free-text intent works (RU+EN) -> purge
```
...
- **`agent_type` for plugin agents** (`brewcode:developer` vs `developer`) has not been observed live. A `byAgentType` key that does not match the real payload value silently falls back to `defaultMinutes` — verify against a real payload before relying on an override.
