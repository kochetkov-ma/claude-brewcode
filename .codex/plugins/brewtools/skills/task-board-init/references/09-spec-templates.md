# 09 -- Step 4b: spec + design document templates

Emitted only when `{{SPEC_MODE}}` is `on`. Write each block below to its path under `TARGET/.codex/features/specs/`. Substitute `{{REPO_NAME}}`, `{{LANG}}`, `{{TODAY}}` (ISO date), `{{FIRST_DOMAIN}}`.

| Emitted file | Copied at runtime to | Filled by |
|--------------|----------------------|-----------|
...
- Scope ids `S1..Sn` come from the task's `## Scope` table (ref 05). !=mint new ones here, !=renumber. That table's `status` cell (`not-started` | `in-progress` | `done`) is the EXECUTION axis, owned by `task-tracker` / `/task-board`: both templates are read-only consumers of it and a `refresh` preserves it verbatim.
- Two question namespaces, never merged: `Q1..Qn` in the spec's `## Open questions`, `AQ1..AQn` in the design's `## Open architectural questions`. Gate G2 scans BOTH documents.
- Prose in {{LANG}}. Ids, headings and table columns stay ASCII as written.
- `status: agreed` is only legal when every `in` scope id reads `covered` in BOTH coverage tables (gate G1).
...
```markdown
---
task: T-{{FIRST_DOMAIN}}-REPLACE-ME
kind: spec
...
# Spec -- T-{{FIRST_DOMAIN}}-REPLACE-ME

> Product spec: HOW we solve it. The task file owns WHAT + WHY. Architecture lives in
> `T-{{FIRST_DOMAIN}}-REPLACE-ME-design.md`. Procedure: [`../TRACKER.md`](../TRACKER.md).

## Summary

*3-5 lines: the ask, the chosen approach, the one thing that could go wrong.*

EXAMPLE: {{REPO_NAME}} needs per-user rate limiting on the public API. We add a token-bucket
...
# Design -- T-{{FIRST_DOMAIN}}-REPLACE-ME

> System design + architecture. Product decisions live in `T-{{FIRST_DOMAIN}}-REPLACE-ME-spec.md`,
> the ask in the task file.

## Context & constraints

*Where this lands in {{REPO_NAME}} and what boxes it in: existing components touched, hard limits (latency, memory, deploy cadence), things that cannot change.*
...
| id | question | blocking | owner |
|----|----------|----------|-------|
| AQ1 | EXAMPLE: fail-open or fail-closed when the cache is down? | yes | {{FIRST_DOMAIN}} domain agent |
...
|--------|-----------------|---------|
| EXAMPLE: {{FIRST_DOMAIN}} | `{{FIRST_DOMAIN}}-expert` | approved; asked for the fail-open row in Failure modes |
| EXAMPLE: infra | none in `.codex/agents/` -- fell back to built-in `Plan` | no blocking objection; flagged the single-node cache as the real limit |
```
