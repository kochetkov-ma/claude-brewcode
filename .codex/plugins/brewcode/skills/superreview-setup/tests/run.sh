#!/usr/bin/env bash
# run.sh - aggregate every brewcode:superreview-setup test suite.
#
...
command -v node >/dev/null 2>&1 || { printf '❌ node is required to run the superreview-setup suites\n' >&2; exit 1; }
