...
Targets: `.codex/agents/*.toml`, `*/agents/*.md` (repo-local only). Disabled files (`_name.md`) are skipped and reported. `$brewdoc:memory-sync-setup` emits a project-local `/memory-sync` skill that applies the same non-growth sync to AGENTS.md, rules, conventions, agents and skills together — use this skill when you want a roster on its own.
